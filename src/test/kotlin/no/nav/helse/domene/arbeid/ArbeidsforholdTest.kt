...
class ArbeidsforholdTest {
    ...
    fun `Jobber faste ukedager`(){
        ...
        assertEquals(syvOgEnHalvTime, perioder[Periode(fredag, fredag)]!!.jobberNormaltTimerPerDag)
        assertEquals(nullTimer, perioder[Periode(fredag, fredag)]!!.faktiskArbeidTimerPerDag)
    }
    ...
    fun `Jobber prosent av normalt`(){
        ...
        perioder.forEach {
            assertEquals(it.value.jobberNormaltTimerPerDag, syvOgEnHalvTime)
            assertEquals(it.value.faktiskArbeidTimerPerDag, syvOgEnHalvTime.dividedBy(2)) // 50%
        }
    }
    ...
    fun `Jobber timer i snitt per uke`(){
        ...
        perioder.forEach {
            assertEquals(it.value.jobberNormaltTimerPerDag, syvOgEnHalvTime)
            assertEquals(it.value.faktiskArbeidTimerPerDag, femTimer)
        }
    }
}